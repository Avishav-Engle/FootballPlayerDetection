...
from our_functions.Our_Functions import my_transforms, BYOL
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                if dataset.mode == 'image':
                  ...
                  state = torch.load('/content/drive/MyDrive/Deep Learning Project/yolov7-main/yolov7-main/our_functions/resnet50_Byol_Best_validation.pt', map_location=device)
                  ...
                  with torch.no_grad():
                      outputs = [byol_model.clustering(tensor_image.to(device))[0].cpu().numpy() for tensor_image in tensor_images]
                  if len(outputs):
                    unique_indexes = [9, 12, 23, 24, 39, 46, 52, 67, 71, 72, 86, 98, 121,124 152 158 173 212
 220 224 231 233 238]
                    kmeans = KMeans(n_clusters=2, random_state=0)
                    player_team = kmeans.fit_predict(outputs)
